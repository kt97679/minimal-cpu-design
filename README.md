...
## Phase 7: searching instead of choosing

Every design above is one I picked, and the ones I picked are the branches that
historically existed — which is a fair charge of bias against a model that has
read that history. So `sw/autosearch.py` searches instead: the instruction pool
is enumerated mechanically (operation x addressing mode, plus a branch for every
way of testing the outcome classes, 31 candidates), code is generated by
breadth-first search rather than hand-written macros, and the Verilog is emitted
from the instruction list and synthesised for real.

It reproduces the hand-written macro expansions exactly and rediscovers the
two-group split without being told it exists — uniform random sampling alone,
about 8,800 draws with no hill climbing, puts index-register machines at
7,505–7,900 modelled gates and self-patching machines at 50,755–87,715, with
nothing in between.

My first pool was not as mechanical as I claimed: I shipped six of the seven
branch conditions, omitting branch-if-not-positive, which is SUBLEQ's own, and
filled the arithmetic slots with operations real machines have. Adding the
missing branch plus reverse subtract and NAND changed the answer.

**The search then beat the hand design.** The best machine has **no ADD, no SUB
and no JMP** — reverse subtract does the work of both, with `a + b` compiled as
`LD d; RSB Kz; RSB s; ST d`. Measured through the same pipeline: **6,959 gates
against 7,161**, 2.8% smaller and 13% slower. It resembles nothing in the
historical record.

Two errors in my own tooling had to be fixed first, both flattering the answer I
already had: a cost model that priced program words at the ROM average, and an
emitter that charged searched machines for a scratch variable they never used.
...
* **[prompts/](prompts/INDEX.md)** — six reusable prompts extracted from this
  ...
  review, and handling the feedback that comes back. Written to be lifted into
  a standalone `ai-prompts` repository and referenced from other projects;
  `prompts/INDEX.md` is the dispatcher and `prompts/USAGE.md` explains the
  wiring.
