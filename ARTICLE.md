...
answer. That objection is fair, and the reply has to be code rather than prose.

So I wrote a search. The instruction pool is enumerated mechanically as
{operation} x {addressing mode} plus a branch for each way of testing the
outcome classes: 31 candidates, none included because a real machine had it.
Code is generated by breadth-first search over instruction sequences rather
than by hand-written macros. The Verilog is emitted from the instruction list
and synthesised for real. Starting points are random subsets, rejected until
one can run the benchmark — about one in twenty.

Two controls came first. The compiler independently reproduced every macro I
had written by hand, including the six-instruction `a + b = a - (0 - b)` for
sets without ADD. And the two-group split reappeared without being told it
exists: every run that chose an index register landed between 6,771 and 6,910
gates, every run that chose self-patching between 50,118 and 59,367. It also
found the ten-instruction machine as a local optimum.

Then it proposed something I would not have: eight instructions with **no ADD
and no JMP**, which every historical accumulator machine has and which I never
thought to question. By the search's own cost model that was 1.1% smaller.

Measured properly, it is 3.4% *larger*: **7,319 gates against 7,078**, and 27%
slower. The search's model priced program words at 4.3 gates, the average over
the ROM image — but the eight-instruction machine needs 38 more words, and
their real marginal cost is 12.2 gates each. Dropping ADD and JMP saves 222
gates of core and costs 463 of memory. This article's own caveat, that the
marginal word is not the average word, is exactly what my search model ignored.

So the hand-designed machine survives — but it now survives a search that tried
to beat it and was refuted by measurement, rather than surviving on my say-so.
That is a better position than it was in before, and it was not the outcome I
expected when I started writing the search.

What remains mine is the skeleton — one accumulator, memory operands, a 16-bit
word — and the two array-access strategies the search chooses between. This is
a search of an instruction space inside an architecture I picked, not a machine
